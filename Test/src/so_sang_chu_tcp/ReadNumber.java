...
public class ReadNumber {
	...
	public static ArrayList<String> readNum(String a) {
		...
		ArrayList<String> List_Number = Split(a, 3);

		while (List_Number.size() != 0) {
			// Xét 3 số đầu tiên của chuổi (số đầu tiên của List_Num)
			switch (List_Number.size() % 3) {
				// 3 số đó thuộc hàng trăm
				case 1:
					kq.addAll(read_3num(List_Number.get(0)));
					break;
				// 3 số đó thuộc hàng nghìn
				case 2:
					ArrayList<String> nghin = read_3num(List_Number.get(0));
					if (!nghin.isEmpty()) {
						kq.addAll(nghin);
						kq.add(NGHIN);
					}
				...
				case 0:
					ArrayList<String> trieu = read_3num(List_Number.get(0));
					if (!trieu.isEmpty()) {
						kq.addAll(trieu);
						kq.add(TRIEU);
					}
			...
			if (List_Number.size() == (List_Number.size() / 3) * 3 + 1 && List_Number.size() != 1)
				kq.add(TY);
	...
	// hàm xuất kq
	public static String xuat(String number) {
